...
import type { Margin as MarginValue, ProductRules, Rule, ProductManagement } from "@/lib/types";
 ...
 *  commission, tax and delivery all taken off. */
function Margin({ value }: { value: MarginValue | null | undefined }) {
  if (!value) return null;
  // Without a purchase price the "profit" is nearly the whole price, which
  // looks like a number but is not one. Ask for the cost instead of showing it.
  if (value.estimated) {
    return <div className="mt-1 text-xs text-slate-400" title="Маржа считается от цены закупки">закуп не указан</div>;
  }
  const profit = Number(value.profit);
  const title = `Цена ${tenge(value.price)} − комиссия ${tenge(value.commission)} − налог ${tenge(value.tax)}`
    + ` − доставка ${tenge(value.delivery)} − закуп ${tenge(value.purchase_price)}`;
  return <div className={`mt-1 text-xs ${profit < 0 ? "text-rose-700" : "text-emerald-700"}`} title={title}>
    {tenge(value.profit)} · {Number(value.margin_percent).toFixed(1)}%
  </div>;
}
  ...
  return <article className="product-card rounded-xl border border-slate-200 bg-white p-3 sm:p-4">
    <header className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs leading-5 text-slate-500">Категория: <button type="button" onClick={openEditor} className="editable-value">{product.category || "не задана"}</button>
          <span className={`ml-2 rounded px-1.5 py-0.5 text-xs ${product.on_sale ? "bg-emerald-50 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{product.on_sale ? "на продаже" : "снят с продажи"}</span>
        </p>
      ...
      <div className="flex shrink-0 gap-1">
        <button onClick={() => setCalculating(true)} aria-label={`Расчёт маржи ${product.sku}`} title="Расчёт маржинальности" className="flex size-9 items-center justify-center rounded-md text-slate-500 hover:bg-slate-50"><Calculator className="size-4" /></button>
    ...
    <dl className="catalog-grid mt-2 rounded-lg bg-[#f7f8f8]">
      ...
      <Detail label="Текущая цена (XML) / Маржа"><span title={market?.observed_price ? `На Kaspi: ${tenge(market.observed_price)}. Проверка: ${relativeTime(rule?.last_evaluated_at ?? null)}` : "Цена для следующей загрузки Kaspi"}><PriceQuickEdit key={`${rule?.id}:${rule?.min_price}:${rule?.max_price}:${rule?.step}`} product={product} rule={rule} /></span><Margin value={product.margins.current} /></Detail>
      <Detail label="Мин. цена / Маржа"><PriceQuickEdit key={`min:${rule?.min_price}:${rule?.max_price}:${rule?.step}`} product={product} rule={rule} field="min_price" /><Margin value={product.margins.minimum} /></Detail>
      <Detail label="Макс. цена / Маржа"><PriceQuickEdit key={`max:${rule?.min_price}:${rule?.max_price}:${rule?.step}`} product={product} rule={rule} field="max_price" /><Margin value={product.margins.maximum} /></Detail>
    ...
    <details className="mt-2 text-xs text-slate-500"><summary className="w-fit cursor-pointer py-1">Подробнее</summary><p className="mt-2 leading-5">Проверка: {relativeTime(rule?.last_evaluated_at ?? null)}. На Kaspi: {tenge(market?.observed_price)}. Маржа посчитана с учётом комиссии Kaspi, налога и доставки из настроек магазина. Автоснижение и автоповышение действуют при включённом боте товара.</p></details>
    {error && <p role="alert" className="mt-3 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">{error}</p>}
    {calculating && <MarginDialog product={product} onClose={() => setCalculating(false)} />}
    {editing && <form className="mt-5 border-t border-slate-200 pt-5" onSubmit={event => { event.preventDefault(); void save({ purchase_price: cost === "" ? null : cost, category: category.trim() || null, ...(stocks.length ? {availabilities: stocks} : {}) }); }}>
      <h3 className="mb-4 font-medium">Закупка и остатки · {product.sku}</h3>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block text-sm text-slate-600">Цена закупки, ₸<input autoFocus type="number" min="0" step="0.01" value={cost} onChange={event => setCost(event.target.value)} className="catalog-input mt-1" placeholder="Не указана" /></label>
        <label className="block text-sm text-slate-600">Категория<input list="product-categories" value={category} onChange={event => setCategory(event.target.value)} className="catalog-input mt-1" placeholder="Например, Системные блоки" /></label>
      </div>
      <label className="mt-3 flex min-h-11 w-fit items-center gap-2 text-sm text-slate-700"><input className="size-5 accent-emerald-600" type="checkbox" checked={product.is_active} disabled={busy} onChange={event => void save({is_active: event.target.checked})} />Товар в прайсе (снимите, чтобы убрать с продажи)</label>
