  ...
  return <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="space-y-4 px-5 py-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm text-slate-700">Минимальная цена, ₸<input type="number" min={1} value={minimum} onChange={(event) => setMinimum(event.target.value)} placeholder="Себестоимость + расходы" className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2" /></label>
        <label className="text-sm text-slate-700">Максимальная цена, ₸<input type="number" min={1} value={maximum} onChange={(event) => setMaximum(event.target.value)} placeholder="Верхняя граница" className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2" /></label>
      </div>
      <label className="block text-sm text-slate-700">Шаг изменения цены, ₸<input type="number" min={1} step={1} value={step} onChange={(event) => setStep(event.target.value)} className="mt-1 block w-28 rounded-lg border border-slate-300 px-3 py-2" /></label>
    ...
    <div className="flex justify-end border-t border-slate-200 px-5 py-4"><button type="button" onClick={save} disabled={saving} className="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{saving && <Loader2 className="size-4 animate-spin" />}Сохранить цены товара</button></div>
  </section>;
}
